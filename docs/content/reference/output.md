...
describes what users get. Pick a format with `-o`, or let comick choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
comick <command> -o table   # aligned columns for reading
comick <command> -o jsonl   # one JSON object per line, for piping
comick <command> -o json    # a single JSON array
comick <command> -o csv     # spreadsheet friendly
comick <command> -o tsv     # tab-separated
comick <command> -o url     # just the URL column
comick <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
comick <command> --fields id,title,url
```
...
```bash
comick <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
comick <command>            # a table, because this is a terminal
comick <command> | wc -l    # JSONL, because this is a pipe
```
